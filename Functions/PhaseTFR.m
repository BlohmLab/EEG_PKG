function TFR = PhaseTFR(S,freqVec,timeVec,Fs,width,TimeInt)
...
%
%%=========================================================================
% Modified to compute the phase angle!
%%=========================================================================
...
for ii=1:size(S,1)          
    for jj=1:length(freqVec)
        t = phasevec(freqVec(jj),detrend(S(ii,:)),Fs,width); % phase
        [b, ~, ~, ~] = cirmean(t(tidx));
        BB(jj,:,ii) = t-b;
        B(jj,:) = (t-b) + B(jj,:);
    end
...
for i = 1:size(BB,2)
    for jj=1:length(freqVec)    
        Bm(jj,i) = cirmean(squeeze(BB(jj,i,:)));
    end
end
TFR = Bm;     
ind = find(TFR<-pi); 
TFR(ind) = TFR(ind)+2*pi;
ind = find(TFR>pi); 
TFR(ind) = TFR(ind)-2*pi;
...
y = atan2(imag(y), real(y));
y(abs(y)<threshold) = 0;
y = y(ceil(length(m)/2):length(y)-floor(length(m)/2));
w = dt:dt:dt*length(y); 
r = atan2(sin(f*2*pi*w),cos(f*2*pi*w));
y = y-r;
